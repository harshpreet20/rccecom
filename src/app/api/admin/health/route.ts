...
export async function GET(request: Request) {
  ...
  const checks: Record<string, { status: "ok" | "pending" | "error"; latency?: number; error?: string }> = {};
  ...
  try {
    ...
    if (isGenerating) {
      checks.brain = { status: "pending", error: "Regenerating" };
  ...
  const allOk = Object.values(checks).every((c) => c.status === "ok" || c.status === "pending");
